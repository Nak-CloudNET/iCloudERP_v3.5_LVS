...
class Sales_model extends CI_Model
{
    ...
    {
		$this->db
			->select("sales.id,sales.note,sales.date as date,erp_quotes.reference_no as q_no, sale_order.reference_no as so_no, 
	...
	public function addSale($data = array(), $items = array(), $payment = array(), $loans = array(),$deliver_id_muti=NULL)
	{
		...
		if ($this->db->insert('sales', $data)) {
			...
			if ($data['sale_status'] == 'completed') {
				$cost = $this->site->costing($items);
				$tran_date = $data['date'];
				$this->site->syncPurchaseItems_delivery($cost, $deliver_id_muti,$tran_date);
    ...
    public function updateSale($id, $data, $items = array(), $sale_data)
    {
        ...
        if ($this->db->update('sales', $data, array('id' => $id))) {
            ...
            if ($data['sale_status'] == 'completed') {
				...
				$cost = $this->site->costing($items);
				$tran_date = $data['date'];
                $this->site->syncPurchaseItems($cost,$tran_date);
            }
    ...
    public function updatePurchaseItem($id, $qty, $sale_item_id, $product_id = NULL, $warehouse_id = NULL, $option_id = NULL, $return_item_id = NULL, $tran_date)
    {
