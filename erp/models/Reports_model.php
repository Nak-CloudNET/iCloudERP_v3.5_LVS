...
*/
   ...
   public function getCustomersByID($id=null, $wh=null) {
	...
	public function getSalesExportByID($id){
		$this->db
                ->select("erp_sales.id, date, reference_no, biller, customer,erp_sales.note,
	...
	public function getWarePur($wid,$warehouse,$product,$category,$biller,$start,$end){
		...
		$this->db->select("erp_warehouses.id,erp_warehouses.name")
				 ...
				 ->where(array('erp_purchase_items.status !='=> 'pending'));
		if ($warehouse) {
	...
	public function getQtyINALL($id,$wid,$tr,$start,$end,$biller){
		//$this->erp->print_arrays($id,"__",$wid);
	...
	public function getProCat($wid,$category2,$product2,$biller,$start,$end){
